...
GPIO_PORTF_DEN_R = 0x0E;
  ...
  while(1){
        ...
        //thirdly,increase the index of the array and zeroing the z again to enable entering a sign or the =    
                     i++; z=0;            
		                 GPIO_PORTF_DATA_R = 0x04;   //it turns on the blue led to inicate that an operand is input. 
                     break;
                case '1':
                     lcd_data(key);
                     no[i]=key;
                 ...
                 if(z==0)     
                  {
                    calculate_operand(no,i-1);			//firstly calculate the operand stored in no[]
                     ...
                     sign=key;
                    GPIO_PORTF_DATA_R= 0x00;         //turns the blue led off when any sign is pressed
                     break;
                  }
                  else
                  {
                 ...
                 if(z==0)
                  {
                    ...
                    GPIO_PORTF_DATA_R= 0x08;    //it turns on the green led when the result is calculated well.
										 delayMs(500);
                    GPIO_PORTF_DATA_R= 0x00;    //it turns off the green led. 
                     delayMs(500);
                     sign=0;
                     i=0; z++;
...
//NVIC_CPAC_CP10_M |= 0x00F00000;
//NVIC_CPAC_CP11_M  |= 0x00F00000;
}
